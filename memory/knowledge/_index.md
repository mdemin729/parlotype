...
| [[benchmark-engine-selection-trap]] | Whisper benchmark configs silently ran whatever engine `settings.json` selected (Parakeet since ADR-041) — the Whisper branch resolved `ISpeechRecognizer` through `DelegatingSpeechRecognizer` instead of a concrete type; Whisper results between ADR-041 and 2026-08-18 are suspect | 2026-08-18 |
