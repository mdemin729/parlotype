...
|------|---------|---------|
| [[whisper-net-quirks]] | Whisper.net 1.9.0 NuGet `CudaHelper` differs from upstream master; `WhisperLogLevel` enum is inverted vs native ggml; `WhisperFactory` has no finalizer, so an undisposed factory leaks the whole model | 2026-07-31 |
