...
- `src/Parlotype.Core/Settings/` — `ISettingsService`, `SettingsKeys` (long-lived, user-configured settings). Deliberately separate: `IWindowStateService`, `WindowStateKeys`, `WindowPosition` (transient window-chrome state — position — that saves far more often, e.g. every drag; never shares a file/lock with `ISettingsService`, ADR-040)
