...
- **Editable prompts** — **Settings → Speech engine → Prompts**. Prompt bodies substitute `{speech_lang}` (the spoken language) and `{text_lang}` (the output language); the built-in default carries purpose-specific bodies for transcription, translation, and auto-detect. The legacy `{language}` token is retired.
