...
- **Parlotype.Benchmark** — Console app for evaluating speech recognition quality. Runs Whisper against WAV/FLAC datasets, computes WER/CER/RTF metrics, outputs JSON results with Spectre.Console tables. Includes SQLite index for historical run queries, comparison engine with delta metrics, parameter sweep support (Cartesian product of config axes), repetition-based stability analysis, per-sample memory/GC tracking, CI regression detection, and export to CSV/Markdown/JSON. Uses `System.CommandLine` for CLI (`run`, `import`, `list`, `compare`, `export`, `check`, `sweep` commands). Subfolders: `Configuration/`, `Metrics/`, `Pipeline/`, `Results/`, `Reporting/`.
